...
@section('content')
    ...
    <section class="content">
        <div class="container-fluid">
            <div class="row">
                <div class="col-12">
                    <form role="form" method="post" action="{{ route('admin.beverages.update', ['beverage' => $beverage->id]) }}" enctype="multipart/form-data">
                        @csrf
                        @method('PUT')
                        <div class="card card-primary card-outline card-tabs">
                            <div class="card-header">
                                <h3 class="card-title">Пункт "{{ $beverage->title_ru }}"</h3>
                            </div>
                            ...
                            <div class="card-body">
                                    ...
                                    <div class="tab-pane fade active show" id="custom-tabs-three-profile" role="tabpanel" aria-labelledby="custom-tabs-three-profile-tab">
                                        <div class="form-group">
                                            ...
                                            <input type="text" name="title_ru"
                                                   ...
                                                   value="{{ $beverage->title_ru }}">
                                        </div>
                                        <div class="form-group">
                                            <label for="subtitle_ru">Подпись на русском</label>
                                            <textarea name="subtitle_ru" class="form-control @error('subtitle_ru') is-invalid @enderror" rows="5" id="subtitle_ru" placeholder="Подпись баннера">{{ $beverage->subtitle_ru }}</textarea>
                                        </div>
                                    </div>
                                    <div class="tab-pane fade" id="custom-tabs-three-home" role="tabpanel" aria-labelledby="custom-tabs-three-home-tab">
                                        <div class="form-group" id="content_en_text">
                                            ...
                                            <input type="text" name="title_en"
                                                   ...
                                                   value="{{ $beverage->title_en }}">
                                        </div>
                                        <div class="form-group">
                                            <label for="subtitle_en">Подпись на английском</label>
                                            <textarea name="subtitle_en" class="form-control @error('subtitle_en') is-invalid @enderror" rows="5" id="subtitle_en" placeholder="Подпись баннера">{{ $beverage->subtitle_en }}</textarea>
                                        </div>
                                    </div>
                                </div>
                                <div class="form-group">
                                ...
                                <div class="d-flex" style="align-items: center;justify-content: space-between">
                                    @if($beverage->image)
                                        <p class="mb-0">
                                            <img style="width:100px;margin-right:10px;border: 2px solid #ddd;padding:2px;" src="{{ str_replace('\\', '/', asset(trim("uploads/$beverage->image"))) }}" alt="{{$beverage->title}}">
                                            <span>{{ str_replace('\\', '/', asset(trim("uploads/$beverage->image"))) }}</span>
                                        </p>
                                    @else
