...
@section('content')
    <section id="banner">
        <div class="banner-images">
            ...
            <img loading="lazy" class="lazyload" data-src="{{ $categories->getImage() }}" alt="{{ $categories->title_ru }}">
        </div>
    ...
    <section id="food" class="drinks">
        <div class="grid">
            <input type="hidden" name="token" value="{{ csrf_token() }}">
            <div class="f-wrap">
                <x-beverage id="{{ $categories->id }}"/>
            </div>
        </div>
    </section>
@endsection
@push('scripts')
    <script>
        ...
        if (qnt >= 6) {
            ...
            $('.category-child-item').css('border-right', '1px solid #dbdbdb')
        }
        ...
        $('.menu-list').on('click', '.category-click', function (event) {
            ...
            $.ajax({
                ...
                url: '{{ route('beverage.update', ['slug' => $categories->slug])  }}',
                data: {
                    category: category,
                    id: "{{ $categories->id }}"
                },
