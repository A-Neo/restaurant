<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        function allPrice () {
            ...
            sum.each(function (index, element) { price += parseInt($(element).text()) });
        ...
        function removerBtnCartPopUp(block) {
            ...
            $.ajax(
                {
                    ...
                    success: function (data)
                    {
                        ...
                        let main_btn = $("a.cart-basket[href^=" + id + "]").closest('.delivery-item__btn');
                        main_btn.animate({ width: '40px' }, 500);
                        main_btn.find('.cart-basket').addClass('cart-none');
                        main_btn.removeClass('cart-active');
                        main_btn.find('.cart-basket').html('<img src="{{ asset('assets/front/img/delivery-cart.svg') }}" alt="Img">');
                    },
                    error: function(result)
                    {
                        console.log(result);
